...
class LabDocument(Base):
    ...
    requires_manual_date: Mapped[bool] = mapped_column(
...
class BiomarkerRecord(Base):
    ...
    lab_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("lab_documents.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    name: Mapped[str] = mapped_column(String(150), nullable=False, index=True)
    raw_name: Mapped[str | None] = mapped_column(String(150), nullable=True)
    ...
    reference_range_raw: Mapped[str | None] = mapped_column(
    ...
    lab_document: Mapped["LabDocument"] = relationship(
        "LabDocument", back_populates="biomarkers"
    )
